...
namespace ShockAnalyze
{
    public class Utils
    {
        ...
        // 十进制转成十六进制
        public static byte[] DecimalToHex(string dec)
        {
            int b = 0;
            for (int i = 0; i < dec.Length; i++)
            {
                int c = 1;
                for (int j = dec.Length - i - 1; j > 0; j--)
                {
                    c = c * 10;
                }

                if (dec[i] > 57 || dec[i] < 48) { break; }
                b = b + (dec[i] - 48) * c;
                c = 1;
            }

            //MessageBox.Show("0x" + BitConverter.ToString(BitConverter.GetBytes((Int16)b)).Replace("-", " 0x"));
            byte[] hex = BitConverter.GetBytes(b);
